...
const dirInport = "pages"
const pathGrav = path.join(__dirname, "pages");
const pathNuxt = {
	img: path.join(__dirname, "nuxt", "assets", "img"),
	content: path.join(__dirname, "nuxt", "content"),
	posters: path.join(__dirname, "nuxt", "posters"),
};
...
const editContentMD = (contentMD, item, dateCreate) => {
	...
	if (metadata.media_order) {
		newMetadata.img = metadata.media_order.split(',')[0];
	} else {
		const pathOldImg = `pages/${item.dir}/`;
		const imgPrimary = getListImagesInDir(pathOldImg)[0];
		newMetadata.img = imgPrimary ? imgPrimary : "";
	}

	if(newMetadata.img) {
		const pathOldImg = path.join(__dirname, `pages/${item.dir}/`);

		fs.copyFile(pathOldImg+newMetadata.img, pathNuxt.posters+"/"+newMetadata.img, (err) => {
			if (err) {
				console.log("Error Found:", err);
			}
			else {
				console.log("Постер скопирован: " + newMetadata.img );
			}
		});
	}
...
list.forEach(item => {
    ...
    const contentMdOut = editContentMD(contentMdIn, item, birthDate(path.join(pathItem, "page.md"), 'YYYY-MM-DDTHH:mm:ss'));
